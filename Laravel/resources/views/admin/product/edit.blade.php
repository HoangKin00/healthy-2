                        ...
                        <div class="card card-info">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="content" class="form-control abc" rows="3" placeholder="Nội dung">
                                        {{old('content') ? old('content') : $product->content}}</textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$img ->id}}</td>
                                                <td>
                                                    <img src="{{ url('public/uploads') }}/{{$img -> image}}" width="70">
                                                    
                                                </td>
                                                <td>
                                                    <a href="{{route('image.edit',$img->id)}}" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a>
                                                    <a href="{{route('product.image-destroy',  $img->id)}}"  class="btn btn-sm btn-danger btn-delete btn-single-delete"><i class="fas fa-trash"></i></a>
                                                    {{-- <form action="{{route('image.destroy',$img->id)}}" method="post">
                                                        @csrf @method('DELETE')
                                                        <a href="{{route('image.edit',$img->id)}}" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a>
                                                    <button style="margin: 0 7px;" onclick="confirm('Bạn muốn xóa ảnh này!')"
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <select name="category_id" class="form-control select2" style="width:100%">
                                        ...
                                        @foreach ($cats as $cat)
                                        <?php 
                                            if(old('categoryProduct_id')){
                                                $selected = old('categoryProduct_id') == $cat->id ? 'selected' : '';
                                            }else {
                                                $selected = $product->categoryProduct_id == $cat->id ? 'selected' : '';
                                            }
                                        ?>
                                ...
                                <div class="form-group">
                                    ...
                                    @enderror
                                    <?php
                                    $pro_img_link = public_path('uploads').'/'.$product->image;
                                    $no_img_link = 'public/admin/dist/img/show.jpg';
                                    $img_link = $product->image ? url('public/uploads').'/'.$product->image : $no_image_link;
