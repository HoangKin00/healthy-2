...
<div class="container">
    <div class="d-flex">
        <a href="{{route('categoryBlog.index')}}" style="float: right" class="btn btn-secondary"><i
    ...
    <a href="{{route('categoryBlog.deleteAll')}} ? is_trashed = true" type="button"
        class="btn btn-danger btn-delete-all ml-2"><i class="fas fa-trash nav-icon"> Xóa lựa chọn </i></a>

    </div>
    <hr>
    <form action="{{route('categoryBlog.restoreAll')}}" method="POST" id="formRestoreAll">
    @csrf @method('PUT')
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <td>
                ...
                <td>
                    <div class="d-flex">
                    <a href="{{route('categoryBlog.restore',$cat->id)}}" title="Phục hồi" class="btn btn-warning" style="margin-right: 10px;"><i class="fas fa-undo-alt"></i></a>
                    <a href="{{route('categoryBlog.forceDelete',$cat->id)}}" onclick="return confirm('Bạn có chắc muốn xóa không?');" class="btn btn-danger btn-delete"><i class="fas fa-trash"></i></a>
                    </div>
                </td>
            </tr>
            @endforeach
...
<form action="{{route('categoryBlog.deleteAll')}}" method="POST" id="formDeleteAll" value="">
    @csrf @method('DELETE')
    <div id="idDelete"></div>
</form>
@stop()
