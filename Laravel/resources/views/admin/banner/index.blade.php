...
<div class="container">
    ...
    <hr>
     <form action="{{route('banner.clear')}}" method="POST" id="formDeleteAll">
        @csrf @method('DELETE')
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                 <td>
                        <div class="icheck-success d-inline">
                            <input type="checkbox" name="id[]" value="{{$ban->id}}" id="item-{{$ban->id}}"
                                class="check_item">
                            <label for="item-{{$ban->id}}">
                            </label>
                        </div>
                    </td>
                ...
                <td>
                    <div class="d-flex">
                        <a href="{{route('banner.edit', $ban->id)}}" class="btn btn-primary" style="margin-right: 10px;"><i class="fas fa-edit"></i></a>
                            <a href="{{route('banner.destroy', $ban->id)}}" class="btn btn-danger btn-delete"><i class="fas fa-trash"></i></a>
                    </div>
                </td>
            </tr>
            @endforeach
