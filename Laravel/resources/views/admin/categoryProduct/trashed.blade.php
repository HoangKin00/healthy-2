...
<div class="container">
    <div class="d-flex">
        <a href="{{route('categoryProduct.index')}}" style="float: right" class="btn btn-secondary"><i
                class="fas fa-arrow-left ml-2"></i>Quay lại</a>

        <a href="" type="button" class="btn btn-warning btn-restore-all ml-2"><i class="fas fa-undo-alt"> Khôi phục lựa
                chọn
            </i></a>
        <a href="{{route('categoryProduct.deleteAll')}} ? is_trashed = true" type="button"
            class="btn btn-danger btn-delete-all ml-2"><i class="fas fa-trash nav-icon"> Xóa lựa chọn </i></a>

    </div>
    <hr>
    <form action="{{route('categoryProduct.restoreAll')}}" method="POST" id="formRestoreAll">
        @csrf @method('PUT')
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div class="icheck-success d-inline">
                            <input type="checkbox" name="id[]" value="{{$cat->id}}" id="item-{{$cat->id}}"
                                class="check_item">
                    ...
                    <td>
                        <div class="d-flex">
                            <a href="{{route('categoryProduct.restore',$cat->id)}}" title="Phục hồi"
                                class="btn btn-warning" style="margin-right: 10px;"><i class="fas fa-undo-alt"></i></a>
                            <a href="{{route('categoryProduct.forceDelete',$cat->id)}}"
...
<form action="{{route('categoryProduct.deleteAll')}}" method="POST" id="formDeleteAll" value="">
    @csrf @method('DELETE')
    <div id="idDelete"></div>
</form>
@stop()
